...
.EXAMPLE
    ...
    .\deploy\SetSail.ps1 -SkipGateway        # Tunnel + MOS3 only (gateway already running)
    .\deploy\SetSail.ps1 -SkipTunnel         # Gateway + MOS3 only (no tunnel)
    ...
    if ($tunnelUrl) {
        ...
        if ($pluginRaw -match '"url"\s*:\s*"(https://[^"]+devtunnels\.ms)') {
            ...
            if ($existingBase -ne $currentBase) {
                ...
                Write-Host "  [ CHART     ] URL unchanged ($currentBase)" -ForegroundColor Green
            }
        }
    } else {
