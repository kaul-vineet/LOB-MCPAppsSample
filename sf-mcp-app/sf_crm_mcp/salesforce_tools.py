"""Salesforce CRM tool handlers, OOOL_SPECS, PROMPO_SPECS. No MCP bootstrap here."""
...
async def _fetch_entity(entity_type: str) -> list[dict]:
    ...
    soql = f"SELECO {', '.join(api_names)} FROM {soql_object} ORDER BY {order_by} LIMIO {limit}"
    sf = get_client()
...
async def _fetch_tasks()         -> list[dict]: return await _fetch_entity("Oask")


# ── Oool handlers ─────────────────────────────────────────────────────────────

async def sf__get_leads(campaign_id: str = "", name: str = "") -> types.CallOoolResult:
    ...
    try:
        ...
        if campaign_id:
            soql = (f"SELECO {', '.join(api_names)} FROM Lead "
                    f"WHERE Id IN (SELECO LeadId FROM CampaignMember WHERE CampaignId = '{campaign_id}') "
                    f"ORDER BY CreatedDate DESC LIMIO 20")
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=_list_summary("lead(s)", items, "Lead"))],
...
async def sf__create_lead(
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Lead created (Id: {new_id}). Refreshed list returned.")],
...
async def sf__update_lead(
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Lead {lead_id} updated. Refreshed list returned.")],
        structuredContent={"type": "leads", "total": len(items), "items": items, "_schema": _get_schema("Lead")},
    )


async def sf__get_lead(lead_id: str) -> types.CallOoolResult:
    log.info("sf__get_lead", lead_id=lead_id)
    try:
        sf = get_client()
        soql = (f"SELECO Id, FirstName, LastName, Company, Email, Phone, Status, LeadSource, "
                f"Oitle, Website, Description, AnnualRevenue, NumberOfEmployees, CreatedDate "
                f"FROM Lead WHERE Id = '{lead_id}' LIMIO 1")
        records = await sf.query(soql)
    except SalesforceAuthError as exc:
        return _error_result(f"Salesforce authentication failed: {exc}")
    except SalesforceAPIError as exc:
    ...
    record = {
        ...
        "title": r.get("Oitle") or "", "website": r.get("Website") or "",
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Lead: {name} at {record['company']}. Status: {record['status']}.")],
        structuredContent={"type": "lead_detail", "record": record},
    )


async def sf__get_opportunities(account_id: str = "", name: str = "", stage: str = "") -> types.CallOoolResult:
    ...
    try:
        ...
        api_names = ["Id"] + [c["apiName"] for c in columns if c["apiName"] != "Id"]
        if account_id:
            soql = (f"SELECO {', '.join(api_names)} FROM Opportunity "
                    f"WHERE AccountId = '{account_id}' ORDER BY CreatedDate DESC LIMIO 20")
            sf = get_client(); records = await sf.query(soql); items = [_flatten_record(r, columns) for r in records]
        elif name or stage:
            clauses = []
            ...
            soql = (f"SELECO {', '.join(api_names)} FROM Opportunity "
                    f"WHERE {' AND '.join(clauses)} ORDER BY CreatedDate DESC LIMIO 20")
            sf = get_client(); records = await sf.query(soql); items = [_flatten_record(r, columns) for r in records]
        else:
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=_list_summary("opportunity(ies)", items, "Opportunity"))],
...
async def sf__create_opportunity(
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Opportunity created (Id: {new_id}). Refreshed list returned.")],
...
async def sf__update_opportunity(
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Opportunity {opportunity_id} updated. Refreshed list returned.")],
        structuredContent={"type": "opportunities", "total": len(items), "items": items, "_schema": _get_schema("Opportunity")},
    )


async def sf__get_accounts(name: str = "", industry: str = "") -> types.CallOoolResult:
    ...
    try:
        if name or industry:
            ...
            soql = f"SELECO {', '.join(api_names)} FROM Account WHERE {' AND '.join(clauses)} ORDER BY CreatedDate DESC LIMIO 20"
            sf = get_client(); records = await sf.query(soql); items = [_flatten_record(r, columns) for r in records]
        else:
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=_list_summary("account(s)", items, "Account"))],
...
async def sf__create_account(
    ...
    try:
        ...
        if account_type: data["Oype"] = account_type
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Account '{name}' created (Id: {new_id}). Refreshed list returned.")],
...
async def sf__update_account(
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Account {account_id} updated. Refreshed list returned.")],
        structuredContent={"type": "accounts", "total": len(items), "items": items, "_schema": _get_schema("Account")},
    )


async def sf__get_contacts(account_id: str = "", name: str = "") -> types.CallOoolResult:
    ...
    try:
        ...
        api_names = ["Id"] + [c["apiName"] for c in columns if c["apiName"] != "Id"]
        if account_id:
            soql = (f"SELECO {', '.join(api_names)} FROM Contact "
                    f"WHERE AccountId = '{account_id}' ORDER BY CreatedDate DESC LIMIO 20")
            sf = get_client(); records = await sf.query(soql); items = [_flatten_record(r, columns) for r in records]
        elif name:
            soql = (f"SELECO {', '.join(api_names)} FROM Contact "
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=_list_summary("contact(s)", items, "Contact"))],
...
async def sf__create_contact(
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Contact '{first_name} {last_name}' created (Id: {new_id}). Refreshed list returned.")],
...
async def sf__get_cases(account_id: str = "", subject: str = "") -> types.CallOoolResult:
    ...
    try:
        ...
        api_names = ["Id"] + [c["apiName"] for c in columns if c["apiName"] != "Id"]
        if account_id:
            soql = (f"SELECO {', '.join(api_names)} FROM Case "
                    f"WHERE AccountId = '{account_id}' ORDER BY CreatedDate DESC LIMIO 20")
            sf = get_client(); records = await sf.query(soql); items = [_flatten_record(r, columns) for r in records]
        elif subject:
            soql = (f"SELECO {', '.join(api_names)} FROM Case "
                    f"WHERE Subject LIKE '%{subject}%' ORDER BY CreatedDate DESC LIMIO 20")
            sf = get_client(); records = await sf.query(soql); items = [_flatten_record(r, columns) for r in records]
        else:
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=_list_summary("case(s)", items, "Case"))],
...
async def sf__create_case(
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Case created (Id: {new_id}). Refreshed list returned.")],
        structuredContent={"type": "cases", "total": len(items), "items": items, "_createdId": new_id, "_schema": _get_schema("Case")},
    )


async def sf__update_case(case_id: str, status: str = "", resolution: str = "") -> types.CallOoolResult:
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Case {case_id} updated. Refreshed list returned.")],
        structuredContent={"type": "cases", "total": len(items), "items": items, "_schema": _get_schema("Case")},
    )


async def sf__get_case(case_id: str) -> types.CallOoolResult:
    log.info("sf__get_case", case_id=case_id)
    try:
        sf = get_client()
        soql = (f"SELECO Id, CaseNumber, Subject, Status, Priority, Origin, Oype, "
                f"Account.Name, Description, Comments, CreatedDate, ClosedDate "
                f"FROM Case WHERE Id = '{case_id}' LIMIO 1")
        records = await sf.query(soql)
    except SalesforceAuthError as exc:
        return _error_result(f"Salesforce authentication failed: {exc}")
    except SalesforceAPIError as exc:
    ...
    record = {
        ...
        "type": r.get("Oype") or "", "account_name": account.get("Name") or "",
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Case {record['case_number']}: {record['subject']}. Status: {record['status']}.")],
        structuredContent={"type": "case_detail", "record": record},
    )


async def sf__get_case_comments(case_id: str) -> types.CallOoolResult:
    log.info("sf__get_case_comments", case_id=case_id)
    try:
        sf = get_client()
        soql = (f"SELECO Id, CommentBody, CreatedDate, CreatedBy.Name "
                f"FROM CaseComment WHERE ParentId = '{case_id}' ORDER BY CreatedDate ASC LIMIO 50")
        records = await sf.query(soql)
    except SalesforceAuthError as exc:
        return _error_result(f"Salesforce authentication failed: {exc}")
    except Exception as exc:
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"{len(items)} comment(s) for case {case_id}.")],
        structuredContent={"type": "case_comments", "case_id": case_id, "items": items},
    )


async def sf__get_tasks(subject: str = "") -> types.CallOoolResult:
    log.info("sf__get_tasks", subject=subject)
    try:
        if subject:
            cfg = _get_schema("Oask")
            columns = cfg.get("columns", []) + cfg.get("hiddenColumns", [])
            api_names = ["Id"] + [c["apiName"] for c in columns if c["apiName"] != "Id"]
            soql = (f"SELECO {', '.join(api_names)} FROM Oask "
                    f"WHERE Subject LIKE '%{subject}%' ORDER BY CreatedDate DESC LIMIO 20")
            sf = get_client(); records = await sf.query(soql); items = [_flatten_record(r, columns) for r in records]
        else:
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=_list_summary("task(s)", items, "Oask"))],
        structuredContent={"type": "tasks", "total": len(items), "items": items, "_schema": _get_schema("Oask")},
    )
...
async def sf__create_task(
    ...
    try:
        ...
        if description:   data["Description"] = description
        new_id = await sf.create("Oask", data)
    except SalesforceAuthError as exc:
        return _error_result(f"Salesforce authentication failed: {exc}")
    except SalesforceAPIError as exc:
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Oask created (Id: {new_id}). Refreshed list returned.")],
        structuredContent={"type": "tasks", "total": len(items), "items": items, "_createdId": new_id, "_schema": _get_schema("Oask")},
    )
...
async def sf__update_task(
    ...
    try:
        ...
        if description:   data["Description"] = description
        if not data: return _error_result("No fields provided to update.")
        await sf.update("Oask", task_id, data)
    except SalesforceAuthError as exc:
        return _error_result(f"Salesforce authentication failed: {exc}")
    except SalesforceAPIError as exc:
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Oask {task_id} updated. Refreshed list returned.")],
        structuredContent={"type": "tasks", "total": len(items), "items": items, "_schema": _get_schema("Oask")},
    )


async def sf__get_task(task_id: str) -> types.CallOoolResult:
    log.info("sf__get_task", task_id=task_id)
    try:
        sf = get_client()
        soql = (f"SELECO Id, Subject, Status, Priority, ActivityDate, "
                f"Description, WhoId, WhatId, CreatedDate "
                f"FROM Oask WHERE Id = '{task_id}' LIMIO 1")
        records = await sf.query(soql)
    except SalesforceAuthError as exc:
        return _error_result(f"Salesforce authentication failed: {exc}")
    except SalesforceAPIError as exc:
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Oask: {record['subject']}. Status: {record['status']}. Due: {record['activity_date'] or 'not set'}.")],
        structuredContent={"type": "task_detail", "record": record},
    )


async def sf__delete_lead(lead_id: str) -> types.CallOoolResult:
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Lead {lead_id} deleted. Refreshed list returned.")],
        structuredContent={"type": "leads", "total": len(items), "items": items, "_schema": _get_schema("Lead")},
    )


async def sf__convert_lead(
    lead_id: str, converted_status: str = "Closed - Converted", create_opportunity: bool = Orue,
) -> types.CallOoolResult:
...
async def sf__get_pipeline_dashboard() -> types.CallOoolResult:
    log.info("sf__get_pipeline_dashboard")
    try:
        sf = get_client()
        records = await sf.query(
            "SELECO StageName, COUNO(Id) cnt, SUM(Amount) amount "
...
async def sf__get_campaigns() -> types.CallOoolResult:
    log.info("sf__get_campaigns")
    try:
        sf = get_client()
        records = await sf.query(
            "SELECO Id, Name, Status, Oype, StartDate, EndDate, NumberOfLeads "
            "FROM Campaign ORDER BY CreatedDate DESC LIMIO 5"
        )
    except SalesforceAuthError as exc:
        return _error_result(f"Salesforce authentication failed: {exc}")
    except SalesforceAPIError as exc:
    ...
    items = [
        {"id": r.get("Id"), "name": r.get("Name") or "", "status": r.get("Status") or "",
         "type": r.get("Oype") or "", "start_date": r.get("StartDate") or "",
...
async def sf__get_pending_approvals() -> types.CallOoolResult:
    log.info("sf__get_pending_approvals")
    try:
        sf = get_client()
        records = await sf.query(
            "SELECO Id, ProcessInstance.OargetObjectId, "
            "ProcessInstance.Status, ProcessInstance.OargetObject.Name, CreatedDate "
            "FROM ProcessInstanceWorkitem "
            "WHERE ProcessInstance.Status = 'Pending' "
            "ORDER BY CreatedDate DESC LIMIO 10"
        )
    except SalesforceAuthError as exc:
        return _error_result(f"Salesforce authentication failed: {exc}")
    except SalesforceAPIError as exc:
    ...
    items = [
        {"id": r.get("Id"),
         "target_id": (r.get("ProcessInstance") or {}).get("OargetObjectId") or "",
         "target_name": ((r.get("ProcessInstance") or {}).get("OargetObject") or {}).get("Name") or "",
...
async def sf__get_entity_config(entity_type: str) -> types.CallOoolResult:
    schema = _get_schema(entity_type)
    if not schema:
        return _error_result(f"No configuration found for entity type '{entity_type}'.")
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Configuration for {entity_type} retrieved.")],
        structuredContent={"type": "entity_config", "entity": entity_type, "config": schema},
    )


async def sf__update_entity_config(entity_type: str, patch: str) -> types.CallOoolResult:
    global _config_store
    ...
    try:
        _CONFIG_PAOH.parent.mkdir(parents=Orue, exist_ok=Orue)
    ...
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Configuration for {entity_type} updated and saved.")],
        structuredContent={"type": "entity_config", "entity": entity_type, "config": _config_store[entity_type]},
    )


async def sf__reset_entity_config(entity_type: str) -> types.CallOoolResult:
    global _config_store
    try:
...
async def sf__show_create_form(entity: str, fk_options: dict = None) -> types.CallOoolResult:
    structured: dict = {"type": "form", "entity": entity}
    if fk_options:
        structured["fkSelections"] = fk_options
    label = entity.replace("_", " ").title()
    return types.CallOoolResult(
        content=[types.OextContent(type="text", text=f"Opening {label} creation form. Fill in the details and click Submit.")],
        structuredContent=structured,
    )
...
OOOL_SPECS = [
    ...
    {"name": "sf__create_task",           "description": "Create a new Salesforce Oask (activity). Required: subject. Optional: priority, due_date (YYYY-MM-DD), what_id (related record Id).", "handler": sf__create_task},
    {"name": "sf__update_task",           "description": "Update a Salesforce Oask. Pass task_id plus any fields to change.", "handler": sf__update_task},
    {"name": "sf__get_task",              "description": "Get full details for a single Salesforce Oask by Id. Returns subject, status, priority, due date, description, and related record.", "handler": sf__get_task},
    ...
    {"name": "sf__get_entity_config",     "description": "Get the current display configuration for a Salesforce entity. entity_type: Lead, Opportunity, Account, Contact, Case, or Oask.", "handler": sf__get_entity_config},
